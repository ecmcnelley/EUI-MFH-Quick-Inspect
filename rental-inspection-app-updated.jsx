...
const RentalInspectionApp = () => {
  ...
  const [expandedSection, setExpandedSection] = useState('housekeeping'); // Track which section is expanded
  ...
  const NSPIRE_REQUIREMENTS = {
    ...
    waterHeater: {
      ...
      temperature: 'Maximum 120Ã‚Â°F at fixtures',
  ...
  const FLOORING_TYPES = ['Carpet', 'Vinyl', 'Vinyl Plank', 'Hardwood', 'Linoleum', 'Cement', 'Tile'];
  ...
  const createRoom = (type, name) => ({
    ...
    drawerTracksNeedRepair: false,
    drawerBoxDamaged: false,
    drawerFaceDamaged: false,
    drawerNotes: '',
    drawerPhotos: [],
  ...
  const createAppliance = (type, brand = '') => ({
    ...
    needsAction: 'None',
    notes: '',
    photos: [], // MUST include: 1 appliance photo + 1 manufacturer sticker photo (2 minimum)
    requiresTwoPhotos: true, // Flag to enforce 2-photo requirement
    workOrder: false,
    ...
    ...(type === 'Water Heater' && {
      ...
      hasDrainPan: false,
      properVenting: true,
      waterTemperature: '120', // Max 120Ã‚Â°F per NSPIRE
      leaks: false
    }),
    ...
    reportWindow.document.write(`
      ...
      <body>
        ...
        <table>
          ...
          <tr><th>Notice Served Date</th><td>${inspectionData.inspectionNoticeServedDate || 'Not specified'}</td><th>Inspection Type</th><td>${inspectionData.inspectionType}</td></tr>
          <tr><th>Program Types</th><td colspan="3">${inspectionData.programType.join(', ')}</td></tr>
        </table>
            ...
            ${room.appliances.map((app, appIdx) => `
              <table style="margin-bottom: 15px;">
                ...
                ${app.type === 'Water Heater' ? `
                  ...
                  <tr><th>Water Temp</th><td>${app.waterTemperature}Â°F</td><th>Leaks</th><td>${app.leaks ? '<span style="color: red; font-weight: bold;">YES</span>' : 'No'}</td></tr>
                ` : ''}
        ...
        ${(() => {
          ...
          if (workOrders.length === 0) {
            return '<p class="alert alert-success"><strong>âœ“ No work orders required.</strong> Unit passed all NSPIRE inspections.</p>';
          }
          
          return `
            <p class="alert alert-warning"><strong>âš  Total Work Orders Required: ${workOrders.length}</strong></p>
            <table>
              <tr>
        ...
        <div class="alert alert-success">
          <p style="margin: 0;"><strong>âœ“ NSPIRE Compliance Declaration:</strong></p>
    ...
    <div className="bg-white shadow-md rounded-lg p-6">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Inspection Notice Served Date <span className="text-red-500">*</span>
            </label>
            <input
              type="date"
              value={inspectionData.inspectionNoticeServedDate}
              onChange={(e) => updateInspectionData('inspectionNoticeServedDate', e.target.value)}
              className="w-full p-2 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500"
              required
            />
          </div>
            ...
            <div className="space-y-3 ml-6">
              ...
              <div className="bg-yellow-50 p-3 rounded text-xs text-yellow-800">
                ...
                <ul className="list-disc ml-4 mt-1">
                  ...
                  <li>Maximum 120Ã‚Â°F at fixtures</li>
  ...
  const renderRoomInspection = (room) => {
    ...
    return (
      <div className="space-y-6">
        ...
        {/* Safety Devices / Alarms - ALL ROOMS */}
            ...
            <div className="bg-white p-3 rounded mb-3">
              <label className="flex items-center space-x-2 mb-2">
                ...
                <span className="text-sm font-semibold">
                  Smoke Alarm Present {(['bedroom', 'hallway'].includes(currentRoom.type)) && '(REQUIRED)'}
                </span>
              </label>
              ...
              {!currentRoom.hasSmokeAlarm && (
                <div className="ml-6 bg-red-100 border border-red-300 p-2 rounded text-sm text-red-800">
                  Ã¢Å¡Â Ã¯Â¸Â NSPIRE VIOLATION: Smoke alarm required in {currentRoom.type}s
                </div>
              )}
            </div>
          ...
          <div className="bg-red-50 border border-red-200 rounded-lg p-4">
            ...
            {!currentRoom.hasGFI && (
              <div className="bg-red-100 border border-red-300 p-2 rounded text-sm text-red-800">
                Ã¢Å¡Â Ã¯Â¸Â NSPIRE VIOLATION: GFI protection required within 6 feet of {currentRoom.type === 'kitchen' ? 'sink' : 'bathroom fixtures'}
              </div>
            )}
                ...
                <>
                  ...
                  {(!currentRoom.windowsOperable || !currentRoom.windowsLocks) && (
                    <div className="bg-yellow-50 border border-yellow-300 p-2 rounded text-sm text-yellow-800">
                      Ã¢Å¡Â Ã¯Â¸Â NSPIRE: All windows must be operable with functioning locks
                    </div>
                  )}
              ...
              <div>
                ...
                {currentRoom.hasDrawers && (
                  <div className="ml-6 space-y-2">
                    ...
                    {/* Drawer Damage Checkboxes */}
                    <div className="bg-gray-50 p-3 rounded border border-gray-200">
                      <p className="text-xs font-semibold text-gray-700 mb-2">Drawer Damage Assessment:</p>
                      
                      <label className="flex items-center space-x-2 mb-2">
                        <input
                          type="checkbox"
                          checked={currentRoom.drawerTracksNeedRepair}
                          onChange={(e) => updateRoom(currentRoom.id, 'drawerTracksNeedRepair', e.target.checked)}
                          className="rounded"
                        />
                        <span className="text-xs">Tracks need repaired/replaced</span>
                      </label>
                      
                      <label className="flex items-center space-x-2 mb-2">
                        <input
                          type="checkbox"
                          checked={currentRoom.drawerBoxDamaged}
                          onChange={(e) => updateRoom(currentRoom.id, 'drawerBoxDamaged', e.target.checked)}
                          className="rounded"
                        />
                        <span className="text-xs">Drawer box damaged</span>
                      </label>
                      
                      <label className="flex items-center space-x-2">
                        <input
                          type="checkbox"
                          checked={currentRoom.drawerFaceDamaged}
                          onChange={(e) => updateRoom(currentRoom.id, 'drawerFaceDamaged', e.target.checked)}
                          className="rounded"
                        />
                        <span className="text-xs">Drawer face damaged</span>
                      </label>
                    </div>
                    
                    {(currentRoom.drawerCondition === 'Fair' || currentRoom.drawerCondition === 'Poor' || 
                      currentRoom.drawerTracksNeedRepair || currentRoom.drawerBoxDamaged || currentRoom.drawerFaceDamaged) && 
          ...
          <>
            ...
            <div className="bg-white border rounded-lg p-4">
              ...
              {!currentRoom.hasVentilation && (
                <div className="bg-red-100 border border-red-300 p-2 rounded text-sm text-red-800">
                  Ã¢Å¡Â Ã¯Â¸Â NSPIRE: Bathroom ventilation required (window or exhaust fan)
                </div>
              )}
              ...
              {currentRoom.riseCount >= 3 && (
                <>
                  ...
                  {!currentRoom.hasRailing && (
                    <div className="bg-red-100 border border-red-300 p-2 rounded text-sm text-red-800">
                      Ã¢Å¡Â Ã¯Â¸Â NSPIRE VIOLATION: Railing required for stairs with {currentRoom.riseCount} rises
                    </div>
                  )}
        ...
        <div className="bg-white border rounded-lg p-4">
          <h4 className="font-semibold text-gray-800 mb-3 flex items-center justify-between">
            <span>Appliances & HVAC</span>
            <button
              onClick={() => {
                const defaultType = currentRoom.type === 'kitchen' ? 'Refrigerator' : 
                                  currentRoom.type === 'laundryRoom' ? 'Washer' : 'Other';
          ...
          {currentRoom.appliances.length === 0 && (
            <p className="text-sm text-gray-500 text-center py-4">
              No appliances added yet. Click "Add Appliance" to add heating units, A/C, or other appliances.
            </p>
          )}
                ...
                <div key={appliance.id} className="border border-gray-300 rounded-lg p-4 bg-gray-50">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="bg-blue-50 border-l-4 border-blue-500 p-3">
                      <p className="text-xs font-semibold text-blue-900 mb-1">📸 REQUIRED: 2 Photos Per Appliance</p>
                      <p className="text-xs text-blue-800">
                        1. Full appliance photo &nbsp;|&nbsp; 2. Manufacturer sticker/label photo showing model & serial
                      </p>
                      {appliance.photos && appliance.photos.length < 2 && (
                        <p className="text-xs text-red-600 mt-1 font-semibold">
                          ⚠ Missing {2 - (appliance.photos?.length || 0)} required photo(s)
                        </p>
                      )}
                    </div>
                    
                    {/* Appliance Photos - Always Show */}
                    {renderPhotoSection(currentRoom, 'appliancePhotos', `Appliance Photos (${appliance.photos?.length || 0}/2 required)`, appliance.id)}
                    ...
                    {appliance.type === 'Water Heater' && (
                      <div className="bg-red-50 p-3 rounded">
                        ...
                        <div className="space-y-2">
                          ...
                          <div>
                            <label className="block text-xs font-medium text-gray-700 mb-1">Water Temperature at Fixture (Ã‚Â°F)</label>
                            <input
                              type="number"
                              max="120"
                              placeholder="Max 120Ã‚Â°F"
                              value={appliance.waterTemperature}
                              onChange={(e) => updateAppliance(currentRoom.id, appliance.id, 'waterTemperature', e.target.value)}
                              className="w-full p-2 border rounded text-sm"
                            />
                            <p className="text-xs text-red-600 mt-1">NSPIRE: Maximum 120Ã‚Â°F</p>
                          </div>
        ...
        <div className="bg-blue-50 border-l-4 border-blue-500 p-4">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            <div>
              ...
              <p className="text-gray-700"><span className="font-medium">Inspection Date:</span> {inspectionData.inspectionDate}</p>
              <p className="text-gray-700"><span className="font-medium">Notice Served:</span> {inspectionData.inspectionNoticeServedDate || 'Not specified'}</p>
